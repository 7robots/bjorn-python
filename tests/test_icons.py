...
def test_lucide_style_emits_font_codepoints():
    ...
    assert icons.for_tag("robotics") == pad_glyph("\ue1bb")
...
def test_nerd_icons_for_known_tags_and_fallback():
    icons = IconSet("nerd")
    assert icons.for_tag("robotics") == pad_glyph("\U000f06a9")
    assert icons.for_tag("#Tech") == pad_glyph(NERD_GLYPHS["code"])
...
def test_config_overrides_and_literal_emoji():
    icons = IconSet("emoji", {"#tech": "terminal", "school": "emoji:🎓", "bogus": ""})
    assert icons.for_tag("tech") == pad_glyph("▸")
    assert icons.for_tag("school") == "🎓 "
    assert icons.for_tag("bogus") == pad_glyph(EMOJI_GLYPHS["tag"])


def test_none_and_auto():
    assert IconSet("none").for_tag("tech") == ""
